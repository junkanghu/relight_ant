...
class VideoComposition:
    def __init__(self, path, format, out_path, out_name):
        self.path = path
        self.format = format
        self.out_path = out_path
        self.out_name = out_name
        
    def composite(self, epoch):
        os.system(f"ffmpeg -framerate 30 -pattern_type glob -i {os.path.join(self.path, self.format)} {os.path.join(self.out_path, str(epoch) + '_' + self.out_name)}")
        # os.system(f"ffmpeg -i {os.path.join(self.path, self.format)} {os.path.join(self.out_path, str(epoch) + '_' + self.out_name)}")
        os.system(f'rm {self.path}/*.png')
        
    def plot(self, img, name):
        img.save(os.path.join(self.path, name.split('.')[0] + '.png'))
